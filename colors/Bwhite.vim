...
"
" Require Optionally:
"   {g:Bwhite_Theme_[s:theme_name]}   <dictionary>  user custom theme palette
"   g:Bwhite_Theme_Options            <dictionary>  user options
"
" Expose:
...
fun! s:acquire_theme_data()
  ...
  if exists("g:Bwhite_Theme") " Users expressed theme preference
    let lowercase_theme_name = tolower(g:Bwhite_Theme)

    if lowercase_theme_name !=? 'default'
      let theme_identifier = 'Bwhite_' . lowercase_theme_name
  ...
  if exists("g:Bwhite_Theme_Options")
    let s:options = g:Bwhite_Theme_Options
  endif
  " }}}
...
"
" Require Optionally:
"   g:Bwhite_Theme_Options  <dictionary>  user option config in .vimrc
"
" Expose:
"   s:langOpt_[LANGUAGE]__[OPTION]  <any>   variables for language options
"
" Example:
"     g:Bwhite_Theme_Options has something like this:
"       'language': {
"       \   'python': {
"       \     'highlight_builtins': 1
"       \   }
...
fun! s:set_color_variables()
  ...
  " Spelling: when spell on and there are spelling problems like this for example: Bwhite. a vim color scheme
...
" @brief command alias for g:Bwhite()
command! -nargs=0 Bwhite :call g:Bwhite()
" }}}
